...
export const OPT_HERO_BACKGROUND_IMAGE = "/images/tutopt.png";

export type OptCategoryItem = {
  id: string;
  name: string;
  slug: string;
...
type OptCompactHeroProps = {
  categories: OptCategoryItem[];
};

export function OptCompactHero({ categories }: OptCompactHeroProps) {
  ...
  return (
    <section className="relative min-h-[300px] overflow-hidden border-b border-blue-200/40 shadow-sm md:h-[300px]">
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-cover bg-no-repeat"
        style={{
          backgroundImage: `url("${OPT_HERO_BACKGROUND_IMAGE}")`,
        ...
        <Container size="lg" className="min-w-0 px-0 sm:px-6 lg:px-8">
          <div className="flex w-full max-w-5xl min-w-0 flex-col gap-3 md:flex-row md:items-center md:gap-4">
            <h1 className="shrink-0 text-lg font-medium leading-tight text-white drop-shadow-sm md:text-xl">
              Ищите <span className="font-bold text-white">оптом</span>
            </h1>

            <SearchWithSuggest
              variant="phrase"
              placeholder="Найти оптовый товар или поставщика..."
              buttonLabel="Найти"
          ...
          <div className="mt-4 grid max-w-5xl grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3 md:flex md:flex-wrap">
            <Button
              className="h-11 w-full rounded-xl bg-white text-blue-700 hover:bg-white/95 active:scale-[0.98] md:w-auto"
              asChild
            >
            ...
            <Drawer open={open} onOpenChange={setOpen}>
              ...
              >
                <DrawerHeader className="border-b border-slate-100 pr-12">
                  ...
                  <DrawerDescription>
                    Выберите раздел опта
                  </DrawerDescription>
                </DrawerHeader>

                <nav
                  aria-label="Категории опта"
                        ...
                        return (
                          <li key={category.id}>
                            <DrawerClose asChild>
                              <Link
                                href={href}
                                className="flex min-h-12 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-800 transition hover:bg-blue-50"
                              >
                                <span
                                  className={cn(
