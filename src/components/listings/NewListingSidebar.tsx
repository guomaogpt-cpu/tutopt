...
import { ListingQualityHints } from "@/components/moderation/ListingQualityHints";
...
type NewListingSidebarProps = {
  ...
  qualityInput?: ListingQualityInput;
  className?: string;
};
...
export function NewListingSidebar({
  ...
  return (
    <aside className={cn("space-y-4", className)}>
      {qualityInput ? <ListingQualityHints input={qualityInput} /> : null}

      <div className="rounded-[22px] border border-[rgba(148,163,184,0.18)] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] dark:border-slate-800 dark:bg-slate-900 dark:shadow-none lg:sticky lg:top-24">
        <div className="flex items-center gap-2 text-[#2563EB]">
          <Lightbulb className="size-5" aria-hidden="true" />
          <h2 className="font-bold text-[#0F172A] dark:text-slate-100">Как сделать объявление лучше</h2>
        </div>
      ...
      <div className="rounded-[22px] border border-[rgba(148,163,184,0.18)] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
        <div className="flex items-center gap-2 text-[#059669]">
          <CheckCircle2 className="size-5" aria-hidden="true" />
          <h2 className="font-bold text-[#0F172A] dark:text-slate-100">Перед публикацией</h2>
        </div>
        <ul className="mt-4 space-y-2.5 text-sm leading-relaxed text-[#64748B] dark:text-slate-400">
      ...
      {hasPreview ? (
        <div className="rounded-[22px] border border-[rgba(148,163,184,0.18)] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
          ...
          <div className="mt-3 overflow-hidden rounded-[18px] border border-[rgba(148,163,184,0.18)] dark:border-slate-800">
            <div className="relative aspect-[4/3] bg-[#F1F5F9] dark:bg-slate-800">
              {imageUrl ? (
                <Image
