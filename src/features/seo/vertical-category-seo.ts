...
export function buildSeoLandingBodyText(
  ...
  switch (category.vertical) {
    case "OPT":
      return `${name} оптом в ${place}: предложения поставщиков, партии товаров, цены и условия продажи. Актуальные объявления для бизнеса на ВсеТут.`;
    case "MARKET":
      return `${name} в ${place}: розничные предложения, частные объявления и товары от продавцов. Смотрите свежие предложения на ВсеТут.`;
    case "SERVICES":
      return `${name} в ${place}: услуги специалистов, мастеров и компаний. Найдите подходящее предложение на ВсеТут.`;
    case "CARGO":
      return `${name} в ${place}: перевозчики, логистика и предложения по доставке. Актуальные объявления на ВсеТут.`;
    default:
      return `${name} в ${place}: актуальные объявления на ВсеТут.`;
  }
}
